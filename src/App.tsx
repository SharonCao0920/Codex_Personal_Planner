...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<string>(tabs.plan)
  ...
  const handleDeleteSection = async (sectionId: string) => {
    ...
    if (activeTab === sectionId) setActiveTab(tabs.plan)
  }
                ...
                {(['todo', 'in_progress', 'done'] as Task['status'][]).map((status) => (
                  <div key={status} className="column">
                    ...
                    <div className="column-list">
                      ...
                      {tasks.filter((task) => task.sectionId === activeSection.id && task.status === status).length ===
                        0 && <p className="muted">No tasks yet.</p>}
                    </div>
                  </div>
                ))}
              </div>
                        ...
                        {entry.status === 'in_progress' && (
                          <div className="checkin-progress">
                            <input
                              ...
                              onChange={(event) =>
                                updatePendingCheckin(task.id, { progress: Number(event.target.value) })
                              }
                            />
